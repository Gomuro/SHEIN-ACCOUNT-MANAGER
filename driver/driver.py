...
import logging
from exceptions import TryAgainPageError, TwitterBotDriverError
from proxy import ProxyABC, Proxy, EmptyProxy
from proxy.connector import ProxyConnectorExtension
from utils import driver_processes
from utils.scrollers import VerticalScroller

class BrowserClosedError(Exception):
    """Exception raised when the browser window is closed unexpectedly."""
    pass

class TwitterBotDriver(uc.Chrome):
    """
        ...
        try:
            ...
            if 'supports chrome version' in str(error).lower():
                ...
                if len(versions) > 1:
                    ...
                    raise TwitterBotDriverError(f"Your Chrome version is: {current_version}<br>"
                                                f"Please install Chrome {supports_version} for stable operation.")
                else:
                    raise TwitterBotDriverError("An error has occurred, likely related to your Chrome version.")
        except TypeError as error:
            if 'binary location must be a string' in str(error).lower():
                raise TwitterBotDriverError("Failed to find Chrome on your PC. Please install Chrome.")
    ...
    def execute_algorithm(self, algorithm, **kwargs):
        # Import here to avoid circular import
        from algorithms import BasicAlgorithm, AlgorithmResult

        self.logger.info('Execute algorithm')
        if isinstance(algorithm, BasicAlgorithm):
            return algorithm.execute(**kwargs)
        else:
            return algorithm(self).execute(**kwargs)
    ...
    def get(self, url: str):
        ...
        Raises:
             - TryAgainPageError when can't pass "Try Again" Twitter page.
        """
        ...
        try:
            ...
            raise TryAgainPageError("There were 10 unsuccessful attempts to pass through the 'Try Again' Twitter page, "
...
if __name__ == '__main__':
    ...
    proxy_string = "194.102.171.121:51523:ivannemforov:FdZNERGcVs"

    proxy = Proxy.from_user_format_string(proxy_string)

    driver = TwitterBotDriver(GLOBAL.PATH.CHROMEDRIVER_PATH, headless=False, proxy=proxy)
    driver.create_instance()

    driver.get('https://twitter.com')
    from utils.bot_humanity import random_sleep

    random_sleep(minimum=60, maximum=120)

    try:
        driver.wait_for_window(timeout=30)
